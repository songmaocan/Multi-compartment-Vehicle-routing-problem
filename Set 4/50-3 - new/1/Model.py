...
class LR:
    def __init__(self):
        ...
        # gap
        self.served_time = []   # 记录服务次数
        self.repeat_served = [] # 重复访问的次数
        self.un_served=[]       #未访问
        self.max_label_cost = float("inf") # 初始上下界用
...
#step1:UB generation
            if i != 0:
                self.served_time[i] = copy.deepcopy(self.served_time[i-1]) #复制上次迭代的服务次数信息
...
# step3: multiplier updating
    ...
    def g_optimal_time_dependent_dynamic_programming(self,vehicle_id,Flag):
        # 准备工作
        self.origin_node=self.virtural_depot # 起终点都是虚拟车场
        ...
        for t in range(self.vehicle_departure_time_beginning, self.vehicle_arrival_time_ending+1):
            ...
            self.g_time_dependent_state_vector[vehicle_id][t] = C_time_indexed_state_vector()# 实例化第一个状态管理集合，跟随路径当前时刻t
        ...
        self.g_ending_state_vector[vehicle_id]=C_time_indexed_state_vector()# 实例化第二个状态管理集合，路径最终点的状态

        max_time_interval = 0
        element = CVSState(self.g_number_of_nodes,self.g_number_of_commodities)# 实例化一条partial path
        ...
        for t in range(self.vehicle_departure_time_beginning, self.vehicle_arrival_time_ending):
            ...
            for index in range(min(len(self.g_time_dependent_state_vector[vehicle_id][t].VSStateVector),self.Best_K_Size)):
                ...
                for i in range(from_node.outbound_size):
                    ...
                    if next_time>self.vehicle_arrival_time_ending: # Case0:超出车场时间窗
                        continue
    ...
    def output_to_file(self, spend_time):
        ...
        with open("Gap.csv", "w") as f:
            f.write("iteration,LB,UB,gap,served_time,Solution_state\n")
            for i in range(self.iteration_times):
                ...
                Flag=1
                for node_id in range(1,self.g_number_of_nodes-1):
                    if served_time[node_id]!=[1]*self.g_number_of_commodities:
                        Flag = 0
                        break
                if Flag==1:
                    f.write("YES")
